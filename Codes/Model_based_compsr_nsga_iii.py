...
# TensorFlow Profiling & Conversion
...
seed_value=1
model_path="/workspace/ExtraTrees_all.pkl" # Change the path 
...
class NSGA3:
    ...
    def _do(self):
        ...
        filename = f"{model_path.split('/')[-1].split('_')[0]}_Output_Population_seed_{seed_value}.txt"
...
class ModifiedMyPMOP:
    def __init__(self, model_path=model_path, n_var=84, n_obj=3): #Just change the model path
        self.n_var = n_var
        self.n_obj = n_obj
    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            print(f"Model loaded fromf {model_path}")
            return model
    ...
    def func_eval_model(self, ind, n_eval):
        ...
        y_pred = self.model.predict(x_ind)
...
print(f"Setting seed to: {seed_value}")# Example usage
